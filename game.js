// Bugman game logic — canvas + audio, no external assets
// ====== Canvas / DPI ======
...
function startMusic(){ ensureAudio(); stopMusic(); musicOn=true; let i=0; musicInterval=setInterval(()=>{ if(!musicOn) return; const step=MELODY[i%MELODY.length]; if(step.n>0){ const now=audioCtx.currentTime; const o=audioCtx.createOscillator(), g=audioCtx.createGain(); o.type='square'; o.frequency.setValueAtTime(step.n,now); g.gain.setValueAtTime(0.001,now); g.gain.linearRampToValueAtTime(0.12,now+0.02); g.gain.exponentialRampToValueAtTime(0.001,now+step.t); o.connect(g).connect(musicGain); o.start(now); o.stop(now+step.t+0.05);} i++; }, 180); }
...
const ghosts=[{name:'Blinky',color:'#ff4b5c',x:13,y:14,dir:'left',speed:0.09},{name:'Pinky',color:'#ff7ad9',x:14,y:14,dir:'right',speed:0.085},{name:'Inky',color:'#00d1d1',x:13,y:15,dir:'up',speed:0.085},{name:'Clyde',color:'#ffb84d',x:14,y:15,dir:'down',speed:0.08}];
...
function buildLevel(){
  ...
  grid=Array.from({length:ROWS},(_,r)=>Array.from({length:COLS},(_,c)=>{
    ...
    return 0; // path by default
  }));
  // fill pellets on paths except ghost house zone
  ...
  // remove unreachable pellets (flood fill from start)
...
btnStart.addEventListener('click',()=>{ ensureAudio(); if(audioCtx.state==='suspended') audioCtx.resume(); playTone(SOUND.start,'triangle'); startOverlay.style.display='none'; canvas.focus(); });
...
addEventListener('keydown',(e)=>{ if(DIR_KEYS[e.key]){ pacman.nextDir=DIR_KEYS[e.key]; e.preventDefault(); } if(e.key===' '){ paused=!paused; btnPause.textContent=paused?'Продолжить ▶':'Пауза ⏸'; if(!paused) loop(); e.preventDefault(); } },{passive:false});

for(const el of document.querySelectorAll('.pbtn')){ el.addEventListener('touchstart',(e)=>{ pacman.nextDir=e.currentTarget.dataset.dir; e.preventDefault(); },{passive:false}); el.addEventListener('mousedown',(e)=>{ pacman.nextDir=e.currentTarget.dataset.dir; }); }
let touchStart=null; canvas.addEventListener('touchstart',(e)=>{ const t=e.changedTouches[0]; touchStart={x:t.clientX,y:t.clientY}; },{passive:true}); canvas.addEventListener('touchend',(e)=>{ if(!touchStart) return; const t=e.changedTouches[0]; const dx=t.clientX-touchStart.x, dy=t.clientY-touchStart.y; if(Math.max(Math.abs(dx),Math.abs(dy))>20){ pacman.nextDir=(Math.abs(dx)>Math.abs(dy))?(dx>0?'right':'left'):(dy>0?'down':'up'); } touchStart=null; },{passive:true});

// ====== Helpers ======
function updateHUD(){ document.getElementById('score').textContent=score; document.getElementById('lives').textContent=Math.max(0,lives); document.getElementById('level').textContent=levelIndex+1; }
function isWallTile(x,y){ return grid[y] && grid[y][x]===1; }
function canMoveFrom(x,y,dir){ const nx=x+DIRS[dir].x, ny=y+DIRS[dir].y; if(nx<0||nx>=COLS||ny<0||ny>=ROWS) return true; return !isWallTile(nx,ny); } // allow exit off-grid (wrap)
function stepActor(a){
  ...
  let nx = x + DIRS[a.dir].x * a.speed;
  let ny = y + DIRS[a.dir].y * a.speed;
...
function ghostAI(g){
  if(frightenedTimer>0){
    if(Math.random()<0.1){
      ...
      let opts=Object.keys(DIRS).filter(d=>canMoveFrom(cx,cy,d) && d!==REVERSE[g.dir]);
...
function update(){
  ...
  if(pacman.alive){
    ...
    if(grid[cy] && (grid[cy][cx]===2 || grid[cy][cx]===3)){
      ...
      if(grid[cy][cx]===3){ frightenedTimer = 600; playTone(SOUND.power,'sawtooth'); } else { if(tick%6===0) playTone(SOUND.chomp,'triangle'); }
      ...
      if(frightenedTimer>0){
        ...
        if(pacman.alive){
          ...
          if(lives<=0){ paused=true; showBanner('Игра окончена','Нажми «Заново», чтобы сыграть снова.'); }
            ...
            Object.assign(pacman,{x:14,y:23,dir:'left',nextDir:'left',alive:true}); frightenedTimer=0;
            [[13,14,'left'],[14,14,'right'],[13,15,'up'],[14,15,'down']].forEach((v,i)=>{ghosts[i].x=v[0];ghosts[i].y=v[1];ghosts[i].dir=v[2];});
          }
        }
      }
...
function nextLevel(){ levelIndex=(levelIndex+1)%LEVELS.length; updateHUD(); pacman.speed+=0.01; ghosts.forEach((g,i)=>g.speed+=0.012+i*0.001); buildLevel(); }
...
function drawMaze(){
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){ if(grid[r][c]===1){ const x=c*TILE,y=r*TILE; const grad=ctx.createLinearGradient(x,y,x,y+TILE); grad.addColorStop(0,'rgba(22,42,112,1)'); grad.addColorStop(1,'rgba(8,18,56,1)'); ctx.fillStyle=grad; ctx.fillRect(x,y,TILE,TILE); ctx.strokeStyle='rgba(98,224,255,.65)'; ctx.lineWidth=2; ctx.strokeRect(x+2,y+2,TILE-4,TILE-4); ctx.shadowColor='rgba(74,109,255,.4)'; ctx.shadowBlur=8; ctx.strokeStyle='rgba(74,109,255,.3)'; ctx.strokeRect(x+3,y+3,TILE-6,TILE-6); ctx.shadowBlur=0; } }
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){ const x=c*TILE+TILE/2, y=r*TILE+TILE/2; if(grid[r][c]===2){ ctx.fillStyle='#ffeaae'; ctx.beginPath(); ctx.arc(x,y,2.6,0,Math.PI*2); ctx.fill(); } else if(grid[r][c]===3){ const t=(tick%60)/60, rad=6+Math.sin(t*2*Math.PI)*2; const glow=ctx.createRadialGradient(x,y,2,x,y,18); glow.addColorStop(0,'rgba(255,210,63,.95)'); glow.addColorStop(1,'rgba(255,210,63,.05)'); ctx.fillStyle=glow; ctx.beginPath(); ctx.arc(x,y,rad,0,Math.PI*2); ctx.fill(); } }
}
function drawActors(){
